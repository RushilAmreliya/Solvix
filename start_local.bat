...
start "NowCast Backend (Port 8000)" cmd /k "python -m uvicorn src.backend.main:app --reload --port 8000"

timeout /t 3 /nobreak >nul

echo [2/3] Starting Data Simulator (60 FPS)...
start "NowCast Simulator (60 FPS)" cmd /k "python -m src.backend.simulator --fps 60 --loop"

timeout /t 2 /nobreak >nul

echo [3/3] Starting React Dashboard on http://localhost:5173...
start "NowCast React Frontend" cmd /k "cd frontend-react && npm run dev"
